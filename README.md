...
アプリコード（`build-logic` を除く `.kt` / `.swift`）9,637 行のうち **96.5% が共有ソースセット**に
...
- **squash マージはしません。** 適切なコミット粒度は `main` の履歴で評価されるものであり、
  その証拠は 1 コミット = 1 つの green な TDD サイクルです。20 個の PR を squash すれば
  `main` には 20 コミットしか残らず、粒度はクローズ済み PR の中に埋まります。
...
この README を書いた時点の `main`: **22 PR / 189 コミット**
（マージコミット 22、通常コミット 167）。内訳は履歴そのものが証拠です。
...
| `build` | 36 | | `test` | 9 |
| `fix` | 21 | | `chore` | 3 |
| `docs` | 20 | | `refactor` / `perf` | 各 1 |

残る 1 件は下記のリポジトリ作成コミットで、Conventional Commits に従う 166 件がその内訳です。
