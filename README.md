...
- **検索は明示的な submit で発火します** — IME の検索アクション、または検索ボタン。1 打鍵ごとではありません。
  ...
  既定の体験になります。
- 二重タップ合体用のデバウンス 800ms（インクリメンタル検索のタイマーではありません）。
  なお**最小クエリ長は 1 文字**で、これはレート制限の対策ではありません。submit 起動では
  クエリ長に関係なく 1 操作 = 1 リクエストであり、`q=k` は GitHub が実際に答える有効な検索です。
  課題が許すキーワードを拒否していた 2 文字の下限は撤回しました
  （[ADR-0015](docs/adr/0015-lower-the-minimum-query-length-to-one-character.md)）。
  ローカルで拒否するのは GitHub 自身が 422 を返す空クエリだけです。
